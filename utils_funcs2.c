#include "shell.h"

/**
 * _strcpy - This function copies a string
 *
 * @dest: It is the destination
 * @src: It is the source
 *
 * Return: It returns the pointer to destination
 */
char *_strcpy(char *dest, char *src)
{
	int i = 0;

	if (dest == src || src == 0)
		return (dest);
	while (src[i])
	{
		dest[i] = src[i];
		i++;
	}
	dest[i] = 0;
	return (dest);
}

/**
 * _strcat - This function concatenates the two strings
 *
 * @dest: It is the destination buffer
 * @src: It is the source buffer
 *
 * Return: returns a pointer to destination buffer
 */
char *_strcat(char *dest, const char *src)
{
	char *ret = dest;

	while (*dest)
		dest++;
	while (*src)
		*dest++ = *src++;
	*dest = *src;
	return (ret);
}

/**
 * _strdup - This is used to function duplicates a string
 *
 * @str: It is the string to duplicate
 *
 * Return: returns a pointer to the duplicated string
 */
char *_strdup(const char *str)
{
	int length = 0;
	char *ret;

	if (str == NULL)
		return (NULL);
	while (*str++)
		length++;
	ret = malloc(sizeof(char) * (length + 1));
	if (!ret)
		return (NULL);
	for (length++; length--;)
		ret[length] = *--str;
	return (ret);
}

/**
 *_putchar - This function is used to write the character c to stdout
 *
 * @c: c is the character to print
 * Return: returns on success 1. On error, -1 is returned,
 * and errno is set appropriately.
 */
int _putchar(char c)
{
	return (write(1, &c, 1));
}

/**
 * _strspn - It is a function that is used to get the
 *           length of a prefix substring
 *
 * @s: "s" is the pointer to string input
 * @accept: This is the substring prefix to look for
 *
 * Return: the function returns number of bytes in the initial segment
 */
unsigned int _strspn(char *s, char *accept)
{
	unsigned int i, j;

	for (i = 0; s[i]; i++)
	{
		for (j = 0; accept[j]; j++)
		{
			if (s[i] == accept[j])
				break;
		}
		if (!accept[j])
			return (i);
	}

	return (i);
}
